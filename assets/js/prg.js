            ...
            $.each(sessions, function(session_id, session) {
                ...
                if (session['type'] == 'Break') {
                    ...
                    if (session['description'] != '') {
                        html += session['description'].replace("####", timestr(new Date(Date.parse(session['start_time']))) + '–' + timestr(new Date(Date.parse(session['end_time']))));
                    }
                    html += '</div></li>';
                } else if (session['type'] == 'Innerbreak') {
                    // Do nothing
                    ...
                    if (session['description'] != '') {
                        html += '<p class="mt-2">' + session['description'] + '</p>';
                    }
                ...
                if (sessions[day_session_index+1] != undefined && sessions[day_session_index+1]['type'] == 'Innerbreak') {
                    var innerbreak = sessions[day_session_index+1];
                    html += '<div class="media-body border bg-lightest my-3 mb-2 py-3 text-center text-muted">';
                    if (innerbreak['description'] != '') {
                        html += innerbreak['description'].replace("####", timestr(new Date(Date.parse(innerbreak['start_time']))) + '–' + timestr(new Date(Date.parse(innerbreak['end_time']))));
                    }
                    html += '</div>';
                }
